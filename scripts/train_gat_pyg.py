#!/usr/bin/env python3
"""
PyTorch Geometric GAT for PlotPointe user–item graph with BPR loss.
...
class PyGGAT(torch.nn.Module):
    def __init__(self, n_users: int, n_items: int, item_feat_dim: int, hidden: int, layers: int, heads: int, attn_dropout: float):
        super().__init__()
        ...
        for _ in range(layers):
            self.convs.append(GATConv(hidden, hidden, heads=heads, dropout=attn_dropout, add_self_loops=False, concat=False))
...
def eval_sampled(model: PyGGAT, cfg: Config, item_feats: torch.Tensor, edge_index: torch.Tensor,
...
def main():
    ap = argparse.ArgumentParser(description="Train PyG GAT")
    ...
    ap.add_argument("--models-prefix", default="gs://plotpointe-artifacts/models/gat/pyg")
    ...
    print("[GAT-PYG] Downloading inputs...")
    ...
    print(f"[GAT-PYG] n_users={n_users}, n_items={n_items}")
    ...
    model = PyGGAT(n_users, n_items, item_feat_dim=item_feats.size(1), hidden=cfg.hidden_dim,
                   layers=cfg.layers, heads=cfg.heads, attn_dropout=cfg.attn_dropout).to(device)
    opt = torch.optim.Adam(model.parameters(), lr=cfg.lr, weight_decay=cfg.l2)

    best_ndcg20 = -1.0
    best_path = "tmp/gat_pyg_best.pt"
    metrics_path = "tmp/metrics_gat_pyg.json"
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        print(f"[GAT-PYG][Epoch {epoch}] loss={loss.item():.4f} ({cfg.loss})")

        model.eval()
        val_metrics = eval_sampled(model, cfg, item_feats, edge_index, train_pos_idx, val_pos_idx)
        print(f"[GAT-PYG][Epoch {epoch}] val: {val_metrics}")
        if getattr(args, "structured_logs", False) and log_event is not None:
            try:
        ...
        if val_metrics.get("ndcg@20", 0.0) > best_ndcg20:
            ...
            print("[GAT-PYG] Saved new best checkpoint")

    ckpt = torch.load(best_path, map_location=device)
    model.load_state_dict(ckpt["state_dict"])
    model.eval()
    test_metrics = eval_sampled(model, cfg, item_feats, edge_index, train_pos_idx, test_pos_idx)
    print(f"[GAT-PYG] test: {test_metrics}")

    out = {"best_val_ndcg@20": float(best_ndcg20), "val": val_metrics, "test": test_metrics,
           "config": cfg.__dict__, "notes": f"One-backward-per-epoch with S sampled BPR triples; features={cfg.item_features}; loss={cfg.loss}"}
    ...
    print(f"[GAT-PYG] \u2705 Complete. Uploaded {chk_gcs} and {met_gcs}")
    ...
    if getattr(args, "use-vertex-exp", False):
        try:
            ...
            with aip.start_run(run=exp_run_name) as run:
                ...
                params.update({"model_family": "gat_pyg", "run_id": run_id, "chk_gcs": chk_gcs, "met_gcs": met_gcs})
            ...
            print("[GAT-PYG] Vertex Experiments logging completed.")
        except Exception as e:
            print(f"[GAT-PYG] Vertex Experiments logging skipped: {e}")
